        ...
        try {
            ...
            if( mList != null && mList.length > 0 ) {
                ...
                if( mList.length == 1 ) {
                    SmbFile f = mList[0];
                    if( f.isDirectory() ) {
                        result.append( "<small>" ).append( ctx.getString( Utils.RR.sz_folder.r() ) ).append( "</small>\n" ).append( f.getName() ).append( "\n" ).append( ctx.getString( R.string.sz_folder_count, num ) ).append( "\n" );
                        if( dirs > 0 )
                        ...
                        result.append( "<small>" ).append( ctx.getString( Utils.RR.sz_file.r() ) ).append( "</small>\n" ).append( f.getName() ).append( "\n" );
                } else
                    result.append( "<small>" ).append( ctx.getString( Utils.RR.sz_files.r() ) ).append( "</small>\n" ).append( num ).append( "\n" );
                if( sum > 0 )
                    result.append( "\n<small>" ).append( ctx.getString( Utils.RR.sz_Nbytes.r() ) ).append( "</small>\n" ).append( Formatter.formatFileSize( ctx, sum ).trim() );
                if( sum > 1024 )
                    result.append( ctx.getString( Utils.RR.sz_bytes.r(), sum ) ).append( "\n" );
                if( mList.length == 1 ) {
                    long last_mod = mList[0].lastModified();
                    if( last_mod > 0 ) {
                        result.append( "\n<small>" ).append( ctx.getString( Utils.RR.sz_lastmod.r() ) ).append( "</small>\n" );
                        String date_s;
                ...
                if( free > 0 && totl > 0 ) {
                    result.append( "<small>" ).append( ctx.getString( Utils.RR.sz_total.r() ) ).append( "</small>\n" ).append( Formatter.formatFileSize( ctx, free ) ).append( "/" ).append( Formatter.formatFileSize( ctx, totl ) );
                }
            }
            sendReport( result.toString() );
        } catch( Exception e ) {
