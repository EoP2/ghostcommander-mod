...
//	final static String URI = "URIfileForEdit";
    ...
    private final static int AXIS_UNDECIDED = 0, AXIS_X = 1, AXIS_Y = 2;
    private int scrollAxis = AXIS_UNDECIDED;
    ...
    public boolean onScroll( MotionEvent ev1, MotionEvent ev2, float distanceX, float distanceY ) {
        ...
        if( scrollAxis == AXIS_UNDECIDED )
            scrollAxis = Math.abs( ev2.getX() - ev1.getX() ) > Math.abs( ev2.getY() - ev1.getY() ) ? AXIS_X : AXIS_Y;
        int newX = te.getScrollX(), newY = te.getScrollY();
        if( scrollAxis == AXIS_X )
            newX += (int) distanceX;
        else
            newY += (int) distanceY;
        int maxX = maxScrollX();
    ...
    public boolean onFling( MotionEvent ev1, MotionEvent ev2, float velocityX, float velocityY ) {
        if( scrollAxis == AXIS_X ) velocityY = 0; else if( scrollAxis == AXIS_Y ) velocityX = 0;
        if( Math.abs( velocityX ) < 1000 && Math.abs( velocityY ) < 1000 ) return false;
        //Log.d( TAG, "onFling " + velocityY + ": " + ev1.toString() + "\n" + ev1.toString()  );
        sc.fling( te.getScrollX(), te.getScrollY(), (int) -velocityX, (int) -velocityY, 0, maxScrollX(), 0, maxScrollY() );
        return true;
    }

    private int maxScrollX() {
        Layout layout = te.getLayout();
        ...
        int visible = te.getWidth() - te.getPaddingLeft() - te.getPaddingRight();
        int max = (int) widest - visible;
        return max > 0 ? max : 0;
    }
    ...
    private int maxScrollY() {
        Layout layout = te.getLayout();
        if( layout == null ) return 0;
        int visible = te.getHeight() - te.getPaddingTop() - te.getPaddingBottom();
        int max = layout.getHeight() - visible;
        return max > 0 ? max : 0;
    }
}
