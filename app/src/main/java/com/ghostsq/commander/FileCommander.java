...
//    private static final Logger log = LoggerFactory.getLogger("FileCommander");    
    ...
    public boolean onCreateOptionsMenu( Menu menu ) {
        ...
        try {
            ...
            if( ab ) {
                ...
                MenuItem more_item = menu.findItem( R.id.action_more );
                if( more_item != null )
                    inflater.inflate( R.menu.menu, more_item.getSubMenu() );
    ...
    public boolean onPrepareOptionsMenu( Menu menu ) {
        ...
        try {
            ...
            int abIconColor = panels.ck.abfgColor;
            int[] ab_icon_ids = { R.id.action_select, R.id.action_nav, R.id.action_more };
            for( int id : ab_icon_ids ) {
                MenuItem mi = menu.findItem( id );
                if( mi != null && mi.getIcon() != null )
                    mi.getIcon().mutate().setTint( abIconColor );
            }
        } catch( Exception e ) {
            Log.e( TAG, "", e );
        }
    ...
    public void dispatchCommand( int id ) {
        ...
        try {
            ...
            if( id == R.id.menu ) {
                View more_view = findViewById( R.id.action_more );
                if( more_view != null )
                    more_view.performClick();
                else
                    openOptionsMenu();
                return;
            }
            ...
            if( id == R.id.hist_back ) {
                panels.histBack( panels.getCurrent() );
                return;
            }
            if( id == R.id.hist_fwd ) {
                panels.histForward( panels.getCurrent() );
                return;
            }
            if( id == R.id.totop ) {
                panels.goTop();
    ...
    private static class SortOption {
        ...
        SortOption( int criterion, CharSequence label ) {
    ...
    private void showSortDialog() {
        ...
        if( ca.hasFeature( CommanderAdapter.Feature.BY_NAME ) )
            options.add( new SortOption( CommanderAdapter.SORT_NAME, getString( R.string.sort_by_name ) ) );
        if( ca.hasFeature( CommanderAdapter.Feature.BY_EXT ) )
            options.add( new SortOption( CommanderAdapter.SORT_EXT,  getString( R.string.sort_by_ext ) ) );
        if( ca.hasFeature( CommanderAdapter.Feature.BY_SIZE ) )
            options.add( new SortOption( CommanderAdapter.SORT_SIZE, getString( R.string.sort_by_size ) ) );
        if( ca.hasFeature( CommanderAdapter.Feature.BY_DATE ) )
            options.add( new SortOption( CommanderAdapter.SORT_DATE, getString( R.string.sort_by_date ) ) );
        SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences( this );
        ...
        if( showAccessDate )
            options.add( new SortOption( CommanderAdapter.SORT_ACCD, getString( R.string.sort_by_adate ) ) );
        if( options.isEmpty() )
            return;

        int currentMode = panels.getSortingMode();
        int checkedIndex = 0;
        for( int i = 0; i < options.size(); i++ )
            if( options.get( i ).criterion == currentMode ) {
                checkedIndex = i;
                break;
            }
        ...
        DialogInterface.OnClickListener applyListener = new DialogInterface.OnClickListener() {
            ...
            public void onClick( DialogInterface dialog, int which ) {
                panels.changeSorting( options.get( selected[0] ).criterion, which == DialogInterface.BUTTON_POSITIVE );
            }
        };
        new AlertDialog.Builder( this )
            ...
            .setPositiveButton( R.string.sort_asc,  applyListener )
            .setNegativeButton( R.string.sort_desc, applyListener )
            .show();
    }
    ...
    private void findSiblingSubtitles( final Uri uri, final Uri ca_uri, final Credentials crd,
        ...
        String ext = Utils.getFileExt( uriStr );
        if( !Utils.str( ext ) ) return;
        final String baseUriStr = uriStr.substring( 0, uriStr.length() - ext.length() );
        String caUriStr = ca_uri.toString();
        String caExt = Utils.getFileExt( caUriStr );
        final String baseCaUriStr = Utils.str( caExt ) ?
            caUriStr.substring( 0, caUriStr.length() - caExt.length() ) : baseUriStr;

        final ArrayList<Uri> foundUris = new ArrayList<>();
        Thread probe = new Thread( "SubtitleProbe" ) {
            @Override
            public void run() {
                CommanderAdapter ca = CA.CreateAdapterInstance( uri, FileCommander.this );
                if( ca == null ) return;
                ca.Init( FileCommander.this );
                ca.setUri( uri );
                ca.setCredentials( crd );
                for( int i = 0; i < SUBTITLE_EXTS.length; i++ ) {
                    String subExt = SUBTITLE_EXTS[i];
                    try {
                        Uri candidate = Uri.parse( baseUriStr + subExt );
                        Item item = ca.getItem( candidate );
                        if( item == null || item.dir ) continue;
        ...
        probe.start();
        try {
            probe.join( SUBTITLE_PROBE_TIMEOUT_MS );
    ...
    private void openRemoteFileFallback( Uri uri, Credentials crd, String scheme, String path ) {
        File temp_file_dir = Utils.getTempDir( this );
        ...
        try {
            ...
            if( progress.what == OPERATION_IN_PROGRESS ) {
                if( progress.arg1 >= 0 ) {
                    ...
                    setSystemOngoingNotification( (int)task_id, string );
                    return CONTINUE;
                } else {
                    if( waitPopup == null )
    ...
    public final void markBgAsUserHidden( long id ) {
        for( NotificationId bg_id : bg_ids ) {
            if( bg_id.is( id ) ) {
                bg_id.userHidden = true;
                return;
            }
        }
    }
    ...
    private void resyncProgressDialog() {
        ...
        if( n_id == null )
            return;
        Dialogs dh = obtainDialogsInstance( Dialogs.PROGRESS_DIALOG );
        dh.setTaskId( n_id.id );
        dh.showDialog();
        remBgNotifId( n_id.id );
    }
    ...
    private void setSystemOngoingNotification( int id, String str ) {
        ...
        if( n_id.last + 1000 > cur_time )
            return;
        n_id.last = cur_time;
        if( n_id.builder == null ) {
            n_id.builder = Build.VERSION.SDK_INT >= Build.VERSION_CODES.O ?
        ...
        n_id.builder.setContentText( str );
        Notification notification = n_id.builder.build();
