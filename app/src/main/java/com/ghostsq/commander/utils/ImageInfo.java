...
public class ImageInfo {
    ...
    public static String getImageInfoAsHTML( ExifInterface exif ) { 
        ...
        if( fe > 0 ) {
            ...
            if( wb != INV ) {
                ...
                if( ws != null ) sb.append( "<br/><small>WB</small><br/>" ).append( ws );
            }
        }
        String ma = exif.getAttribute( ExifInterface.TAG_MAKE );
        if( ma != null ) sb.append( "<br/><small>Make</small><br/>" ).append( ma );
        String mo = exif.getAttribute( ExifInterface.TAG_MODEL );
        if( mo != null ) sb.append( "<br/><small>Model</small><br/>" ).append( mo );
        String dt = exif.getAttribute( ExifInterface.TAG_DATETIME );
        if( dt != null ) sb.append( "<br/><small>Date</small><br/>" ).append( dt );
        return sb.toString();
    }
    ...
    public static void getImageFileExtraInfo( ExifInterface exif, StringBuilder sb ) {
        ...
        if( ap != null ) sb.append( "<br/><small>Aperture</small><br/>f" ).append( ap );
        String ex = exif.getAttribute( ExifInterface.TAG_EXPOSURE_TIME );
        if( ex != null ) {
            try {
                ex = new DecimalFormat("###.#####").format( Double.parseDouble( ex ) );
            } catch( Exception e ) {}
            sb.append( "<br/><small>Exposure</small><br/>" ).append( ex ).append( "s" );
        }
        String fl = exif.getAttribute( ExifInterface.TAG_FOCAL_LENGTH );
        if( fl != null ) sb.append( "<br/><small>Focal length</small><br/>" ).append( fl );
        String is = exif.getAttribute( ExifInterface.TAG_ISO );
        if( is != null ) sb.append( "<br/><small>ISO level</small><br/>" ).append( is );
    }
