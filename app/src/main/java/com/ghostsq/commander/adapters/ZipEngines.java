...
//                    return;
    ...
    public static class CalcSizesEngine extends EnumEngine {
        ...
        public void run() {
            ...
            if( totalSize > 0 )
                result.append( "\n<small>" ).append( c.getString( R.string.sz_Nbytes ) ).append( "</small>\n" ).append( Formatter.formatFileSize( c, totalSize ).trim() );
            if( totalSize > 1024 )
                result.append( c.getString( R.string.sz_bytes, totalSize ) ).append( "\n" );
            result.append( "\n<small>" ).append( c.getString( R.string.zip_compressed ) ).append( "</small>\n" );
            result.append( Formatter.formatFileSize( c, totalCompressed ).trim() ).append( "\n" );
            if( mList.length == 1 ) {
                FileHeader f = mList[0];
                String name_fixed = owner.fixName( f );
