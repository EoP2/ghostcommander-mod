...
public class ListHelper {
    ...
    private final static int HIST_CAP = 30;
    ...
    private final static int RECENT_CAP = 30;
    private final LinkedList<HistEntry> recentList = new LinkedList<HistEntry>();
    ...
    private void recordVisit( Uri target ) {
        try {
            HistEntry cur = captureCurrentEntry();
            if( cur == null ) return;
            if( target != null && Utils.addTrailingSlash( cur.uri ).equals( Utils.addTrailingSlash( target ) ) )
                return;
            Iterator<HistEntry> it = recentList.iterator();
            while( it.hasNext() ) {
                if( Utils.addTrailingSlash( it.next().uri ).equals( Utils.addTrailingSlash( cur.uri ) ) ) {
                    it.remove();
                    break;
                }
            }
            recentList.addFirst( cur );
            while( recentList.size() > RECENT_CAP )
                recentList.removeLast();
        } catch( Exception e ) {
            Log.e( TAG, "recordVisit()", e );
        }
    }
    ...
    public final ArrayList<HistEntry> getRecentList() {
        return new ArrayList<HistEntry>( recentList );
    }
    ...
    public final void restoreRecentList( ArrayList<HistEntry> recent ) {
        if( recent == null )
            return;
        recentList.clear();
...
//        flv.setCacheColorHint( ck.bgrColor );
        if( !ck.isFocusDefault() ) {
            Drawable selector_drawable = new ColorDrawable( ck.curColor );
    ...
    public final String getActiveItemsSummary( boolean touched ) {
        ...
        if( counter != 1 )
            return Utils.getNItems( p.c, counter );
        ...
        for( int i = 0; i < cis.size(); i++ ) {
            if( cis.valueAt( i ) ) {
                ...
                if( !Utils.str( item_name ) )
                    item_name = ca.getItemName( pos, true );
    ...
    public void updateStatus() {
        ...
        String ais = getActiveItemsSummary( false );
