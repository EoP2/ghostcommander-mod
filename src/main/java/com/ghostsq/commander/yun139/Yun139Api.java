...
public class Yun139Api {
    ...
    public Yun139Api(Context appCtx) {
        this.appCtx = appCtx;
        SharedPreferences sp = appCtx.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
        authorization = sp.getString(KEY_AUTHORIZATION, null);
        account = sp.getString(KEY_ACCOUNT, null);
    }

    public static void saveAuthorization(Context appCtx, String rawToken) {
        ...
        if (rawToken != null && rawToken.trim().length() > 0) {
            ed.putString(KEY_AUTHORIZATION, rawToken.trim());
        } else {
            ed.remove(KEY_AUTHORIZATION);
        }
        ed.remove(KEY_ACCOUNT); // will be re-derived from the (new) token on next use
        ed.apply();
    }

    public static String loadAuthorization(Context appCtx) {
        return appCtx.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE).getString(KEY_AUTHORIZATION, null);
    }

    public static void clear(Context appCtx) {
        appCtx.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE).edit().clear().apply();
    }
    ...
    public synchronized void ensureReady() throws IOException {
        if (!hasToken())
            throw new Yun139Exception("尚未配置 139 云盘授权令牌");
    ...
    private void persist() {
        SharedPreferences.Editor ed = appCtx.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE).edit();
        ed.putString(KEY_AUTHORIZATION, authorization);
    ...
    private void refreshTokenIfNeeded() throws IOException {
        ...
        try {
            ...
            throw new Yun139Exception("授权令牌不是合法的 Base64 文本，请重新粘贴");
        }
        String decodedStr;
        try {
            decodedStr = new String(decoded, "UTF-8");
        } catch (UnsupportedEncodingException e) {
            throw new Yun139Exception("授权令牌解码失败");
        }
        String[] splits = decodedStr.split(":");
        if (splits.length < 3)
            throw new Yun139Exception("授权令牌格式不正确（应形如 appId:手机号:token|...）");
        account = splits[1];
        String[] strs = splits[2].split("\\|");
        if (strs.length < 4)
            throw new Yun139Exception("授权令牌格式不正确（token 段缺少过期时间）");
        long expiration;
        try {
            expiration = Long.parseLong(strs[3]);
        } catch (NumberFormatException e) {
            throw new Yun139Exception("授权令牌格式不正确（过期时间不是数字）");
        }
        ...
        if (!"0".equals(ret)) {
            ...
            throw new Yun139Exception("令牌刷新失败：" + (desc != null && desc.length() > 0 ? desc : ("code=" + ret)));
        }
        String newToken = xmlTag(respXml, "token");
        if (newToken == null || newToken.length() == 0)
            throw new Yun139Exception("令牌刷新失败：服务器未返回新令牌");
        try {
            authorization = Base64.encodeToString((splits[0] + ":" + splits[1] + ":" + newToken).getBytes("UTF-8"), Base64.NO_WRAP);
        } catch (UnsupportedEncodingException e) {
            throw new Yun139Exception("令牌刷新失败：编码错误");
        }
        persist();
    }
